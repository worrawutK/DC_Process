...
Public Class MDIParent1
    ...
    ' APCS Test Server  Data Source=10.28.33.11;Initial Catalog=APCSDB;Persist Security Info=True;User ID=sa;Password=p@$$w0rd
    ' APCS Server    Data Source=172.16.0.102;Initial Catalog=APCSDB;Persist Security Info=True;User ID=apcsdbuser

#Region "Commomn Define"

    Dim WithEvents MainControlfrm As New Form1
    Dim WithEvents Alarmfrm As New AlarmTable
    ...
    Dim WithEvents FrmInput As FormInputData
    Dim WithEvents FrmInputQR As FormInputQRCode
    'Event rs()
...
#End Region
    ...
    Private Sub MDIParent1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            '  Me.WindowState = FormWindowState.Maximized
            Alarmfrm.MdiParent = Me
            Alarmfrm.Width = Me.Width - 5
...
#Region "SecsGem"
    ...
    Private Sub m_CommLog(ByVal sender As Object, ByVal e As TraceLogEventArgs)  '160930 \783 Revise FrmSecsDisplay
        ...
        Catch ex As Exception
            SaveCatchLog(ex.Message, "m_CommLog()")
        End Try
    End Sub
    ...
    Private Sub m_Host_ReceivedPrimaryMessage(ByVal sender As Object, ByVal e As PrimarySecsMessageEventArgs)
        ...
        'S6F3 and S64F9 must be queued !!! (WORK_START and WORK_END of those good modules will be execute when S6F3 and S64F9)
        ''        If (msg.Stream = 6 AndAlso msg.Function = 3) OrElse (msg.Stream = 64 AndAlso msg.Function = 9) Then
        ''            '1.) Reply ACKNOWLEDGE
        ''            If TypeOf msg Is S6F3 Then
        ''                Dim reply As S6F4 = New S6F4()
        ''                m_Host.Reply(msg, reply)
        ''            ElseIf TypeOf msg Is S64F9 Then
        ''                Dim reply As S64F10 = New S64F10()
        ''                m_Host.Reply(msg, reply)
        ''            End If
        ''            '2.) Force all S6F3 and S64F9 must be executed by order using MessageQueue
        ''            '    because WORK_START should be executed before WORK_END
        ''            '    (WORK_START is on S6F3 and WORK_END is on S64F9)
        ''TRY_MONITOR:
        ''            If Monitor.TryEnter(m_Locker, 1000) Then
        ''                m_MessageQueue.Enqueue(msg)
        ''                If m_MessageQueueThread Is Nothing OrElse Not m_MessageQueueThread.IsAlive Then
        ''                    'update display
        ''                    Try
        ''                        m_Display.UpdateThreadProcessingStateThreadSafe(m_MessageQueue.Count)
        ''                    Catch ex As Exception
        ''                    End Try
        ''                    'start S6F3 thread
        ''                    m_MessageQueueThread = New Thread(AddressOf ProcessMessageQueue)
        ''                    m_MessageQueueThread.CurrentCulture = m_ICIM_CULTUREINFO
        ''                    m_MessageQueueThread.Priority = ThreadPriority.BelowNormal
        ''                    m_MessageQueueThread.Start()
        ''                End If
        ...
        ''        ElseIf msg.Stream = 64 Then
        ''            Dim t As Thread = New Thread(AddressOf ProcessSecsMessage)
        ''            t.Priority = ThreadPriority.AboveNormal
        ''            t.CurrentCulture = m_ICIM_CULTUREINFO
        ''            t.Start(msg)
        ''Else
        'OTHER commands no need to be in queue
        'and they are not have fequency as S6F3


        ProcessSecsMessage(msg)    '160802 \783 Debug for S6F12 do not reply
        ''ThreadPool.QueueUserWorkItem(AddressOf ProcessSecsMessage, msg)
        ''End If
    End Sub
...
FinalExeLoop:
                ...
                Case 5
                    Select Case msg.Function
                        Case 1 'Alarm Report Send
                            Dim S5F1 As New S5F1
                            S5F1 = DirectCast(msg, S5F1)
                            Reply_S5F1(CType(msg, S5F1))

                            Try
                                If S5F1.AlarmCode = 0 Then 'alarm Clear
                                    FrmAlarmShow(False, CStr(S5F1.AlarmID), S5F1.AlarmText, "0", True)
                                Else
                                    FrmAlarmShow(True, CStr(S5F1.AlarmID), S5F1.AlarmText, "0", True)
                            ...
                            'If FrmProdTable IsNot Nothing Then  '\783 160916 Alarm table add
                            '    If S5F1.AlarmCode = 0 Then 'alarm Clear
                            '        MDIAlarmTable(False, CStr(S5F1.AlarmID), S5F1.AlarmText)
                            '    Else 'alarm set
                            '        MDIAlarmTable(True, CStr(S5F1.AlarmID), S5F1.AlarmText)
                            '    End If
    ...
    Private Sub HostSend(ByVal msg As SecsMessageBase) Handles FrmSecs.E_HostSend, FrmProduct.E_HostSend
        Try
            If CommuniationState = "NOT_SELECTED" Then     '160627 \783 Eq comm revise
                slMessage.Text = "HSMS Data Message Activity can not do now.  Now Comm State is " & CommuniationState & Format(Now, " |HH:mm:ss.fff")
                SaveCatchLog(slMessage.Text, "HostSent()")
                Exit Sub
            End If
            If CommuniationState = "NOT_CONNECTED" Then     '160628 \783 Eq comm revise
...
#Region "TDC"
    ...
    Private Sub LotRequest_RunWorkerCompleted(ByVal sender As Object, ByVal e As RunWorkerCompletedEventArgs)
        Dim result As New TdcLotRequestResponse
        result = CType(e.Result, TdcLotRequestResponse)

        If OprData.FRMProductAlive Then
            FrmProduct.LR_Reply(result, m_TdcService)
        End If
    ...
    Private Sub LotSet_RunWorkerCompleted(ByVal sender As Object, ByVal e As RunWorkerCompletedEventArgs)
        ...
        If OprData.FRMProductAlive Then
            FrmProduct.LS_Reply(result, m_TdcService)
        End If
    ...
    Private Sub LotEnd_RunWorkerCompleted(ByVal sender As Object, ByVal e As RunWorkerCompletedEventArgs)
        ...
        result = CType(e.Result, TdcResponse)
        If OprData.FRMProductAlive Then
            FrmProduct.LE_Reply(result, m_TdcService)
        End If
...
#Region "UserInterface"
    ...
    Public Sub AddAlarmT(ByVal ALCD As Boolean, ByVal ALID As String, ByVal ALTX As String, ByVal AlarmType As String, WaferFabLotNo As String, WaferNo As String, Optional update As Boolean = False)
        If update = True Then


            If ALCD Then

                Dim AlarmID As Integer? = Alarmfrm.GetAlarmIDAdapter1.SearchAlarmID(My.Settings.MCType, CInt(ALID))
                If AlarmID Is Nothing Then
                    'not Alarm
                    Exit Sub
                End If

                Dim Row As DBxDataSet.DCAlarmInfoRow = Alarmfrm.DBxDataSet.DCAlarmInfo.NewDCAlarmInfoRow
                Row.AlarmID = CInt(AlarmID)
                Row.WaferFabLotNo = WaferFabLotNo
                Row.WaferNo = WaferNo
                Row.RecordTime = CDate(Format(Now, "yyyy-MM-dd HH:mm:ss"))
                Row.MCNo = "BG-" & My.Settings.EquipmentNo
                Row.AlarmMessage = ALTX
                Alarmfrm.DBxDataSet.DCAlarmInfo.AddDCAlarmInfoRow(Row)
            Else
                For Each data As DBxDataSet.DCAlarmInfoRow In Alarmfrm.DBxDataSet.DCAlarmInfo
                ...
                Next
            End If
            Alarmfrm.DCAlarmInfoTableAdapter.Update(Alarmfrm.DBxDataSet.DCAlarmInfo)
    ...
    Private Sub FrmAlarmShow(ByVal AlarmALCD As Boolean, ByVal AlarmALID As String, ByVal AlarmALTX As String, Optional ByVal AlarmType As String = "0", Optional AddAlarm As Boolean = False) Handles FrmProduct.AlramShow
        'If AddAlarm = True Then
        '    Alarmfrm.AddAlarm(AddAlarm)
        'Else
        '    Alarmfrm.Timer1.Enabled = False            'Reset Timer1 
        '    Alarmfrm.Timer1.Enabled = True
        'End If
        If FrmProduct.lbDCStartTime.Text <> "" AndAlso FrmProduct.lbDCEndTime.Text = "" Then

        Else
            Alarmfrm.DBxDataSet.DCAlarmInfo.Clear()
            Exit Sub
        End If

        Dim Frm As Form = Me.ActiveMdiChild
        If Not Frm.Name = "AlarmTable" Then

            Me.Alarmfrm.Activate()
            If Alarmfrm.WindowState = FormWindowState.Minimized Then

                Alarmfrm.WindowState = FormWindowState.Normal
                Alarmfrm.Dock = DockStyle.Bottom


                AddAlarmT(AlarmALCD, AlarmALID, AlarmALTX, AlarmType, FrmProduct.lbDCWFLotNo.Text, FrmProduct.lbDCWaferNo.Text, AddAlarm)



            Else
                Alarmfrm.Dock = DockStyle.None
                Alarmfrm.WindowState = FormWindowState.Minimized

            End If
            Alarmfrm.Timer1.Stop()
            Exit Sub
        End If
...
show:
    ...
    Private Sub QR_DataRead() Handles FrmHelper.E_QRReadSuccess  ' QR DATA CHECK
        slMessage.Text = "QR Slip Read Success_" & Format(Now, " |HH:mm:ss.fff")
        FrmProdTable.Close()
        ...
        '            EndLot = True
        '        End If
        '        frmInputData.GetBGDatatable(frmInputQR.m_BGTable, rowNum, EndLot, False)
        '        frmInputData.ShowDialog()
        '        rowNum += 1
        '    Next

        '    ' OprData.QrData = ""
        '    'frmInputQR.m_BGTable
        'End If
        ...
        If My.Settings.UserAuthenOP = "NOUSE" Then  'Bypass Auten
            ...
            OprData.PermitCheckResult = "Permission by pass"
            GoTo BypassAuter
        End If

        If Not FrmProduct.PermiisionCheck(CellConTag.DeviceName, OprData.OPID, My.Settings.UserAuthenOP, My.Settings.UserAuthenGL, My.Settings.ProcessName, My.Settings.EquipmentNo) Then   '170403 \783 Change QRData to Device(bug)
...
BypassAuter:
        ...
        FrmProduct.Loaddata()
        ' FrmProduct.QrSlipOK() 'Read QR
        'If FrmInput Is Nothing Then
        '    FrmInput = New FormInputData
        '    ' GoTo show
        'End If
        'If FrmInput.IsDisposed Then
        '    FrmInput = New FormInputData
        '    '   GoTo show
        'End If
        ...
        'FrmInput.Show()
        'FrmInput.Dock = DockStyle.Bottom

        'FrmHelper.pbxWorkSlipBorder.BackColor = Color.Transparent
        'FrmHelper.pbxInputBorder.BackColor = Color.Blue
        'Dim inputQty As New INPUTQty
        'Dim Result As New List(Of String)
        'Result = inputQty.InputQtyShowDialog  '0 > InputQty , 1 > TDC Start Mode
        'FrmProduct.lbInputTotal.Text = Result(0)
        'Dim TDCStartMode As New RunModeType
        'TDCStartMode = CType(Result(1), RunModeType)
        'OprData.LSMode = TDCStartMode
        'FrmHelper.pbxInputBorder.BackColor = Color.Transparent


        'If My.Settings.WaferMappingUse Then
        '    If Not FrmProduct.WaferMapReadFromZion() Then
        '        GoTo LoopEnd                                           'Laod Map err goto LoopEnd
        '    End If
        'End If
        'FrmProduct.DataInitialSucess(WorkSlipQR)
        'FrmHelper.Close()
...
LoopEnd:
    ...
    End Sub
    Private Sub TextMessage(Txt As String) Handles FrmProduct.MessageFrmMain ', FrmInputQR.MessageError
        'slMessage.Text = "Please Read OPID QR Data" & Format(Now, " |HH:mm:ss.fff")
        slMessage.Text = Txt & Format(Now, " |HH:mm:ss.fff")
    End Sub
    ...
    Private Sub WorkSlipDataClear(SlipType As String) Handles FrmHelper.E_WorkSlipClick
...
show:
    ...
    Private Sub LoadFrFile() Handles FrmSecs.E_LoadFrFile
        ...
        Try
            ...
            If DTable.Rows(0)("MachineType").ToString <> My.Settings.MCType Then          '170328 \783 check machine type before load from file
                slMessage.Text = "Load Err Machine type in Setting difference from File" & Format(Now, " |HH:mm:ss.fff")
                Exit Sub
            End If
...
NextS:
    ...
    '    Dim screenWidth As Integer = Me.Width ' Screen.PrimaryScreen.Bounds.Width
    '    Dim screenHeight As Integer = Me.Height 'Screen.PrimaryScreen.Bounds.Height
    '    ToolStripStatusLabel2.Text = "W :" & screenWidth & "/ H :" & screenHeight
    'End Sub

    Private Sub NormalToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles NormalToolStripMenuItem1.Click
        Me.WindowState = FormWindowState.Normal
        ' RaiseEvent rs()
        FrmProduct.TimerResize1.Start()
    End Sub
    ...
    Private Sub MDIParent1_BackgroundImageChanged(sender As Object, e As EventArgs) Handles Me.BackgroundImageChanged

    End Sub

    'Private Sub TestToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles TestToolStripMenuItem.Click
    '    FrmAlarmShow(False, "", "", "0", False)
    'End Sub

    Private Sub UpdateToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles UpdateToolStripMenuItem.Click
        '  FrmAlarmShow(True, CStr(5000), "555", "0", True)
        FrmAlarmShow(False, "", "", "0", False)
    End Sub
