        ...
        For Each ctl As Control In thisCtrl.Controls
            Try
                ...
                If ctl.GetType Is GetType(PictureBox) Then
                    Dim Pl As PictureBox = CType(ctl, PictureBox)
                    'PictureBox      Dim Pl As Panel = CType(ctl, Panel)
                    'If Pl.BackColor = Color.Transparent Then
                    '    Pl.BackColor = Color.White
                    'End If
                    If Pl.Name <> "pbxLogo" Then
                        Exit For
                    End If
                    Dim parentHeight = ctl.Parent.Height
                    Dim parentWidth = ctl.Parent.Width
                    ...
                    Try
                        ...
                        If (ret) Then
                            '-- Size
                            SizeLoGo = ctl.Width / ctl.Height
                            ctl.Width = SizeLoGo * Int(parentHeight * c.heightPercent) 'Int(parentWidth * c.widthPercent)
                            ctl.Height = Int(parentHeight * c.heightPercent)

                            ''-- Position
                            ctl.Top = Int(parentHeight * c.topOffsetPercent) '* c.widthPercent
                            ctl.Left = Int(parentWidth * c.leftOffsetPercent) ' - (Int(parentHeight * c.heightPercent) - (SizeLoGo * Int(parentHeight * c.heightPercent))) '* c.heightPercent
                ...
                Else
                    If ctl.GetType Is GetType(Label) Then
                        Dim Pl As Label = CType(ctl, Label)
                        If Pl.Name = "Label63" Then
                            Dim x = ""
                        End If
                    End If
